...
class backup_qtype_matrix_plugin extends backup_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        ...
        $weight_sql .= 'm.id= ? AND w.rowid=r.id AND w.colid=c.id AND c.matrixid=m.id AND r.matrixid=m.id';
        $matrix_weight->set_source_sql($weight_sql, array(backup::VAR_PARENTID));
        //$matrix_weight->set_source_sql($weight_sql, array(backup::VAR_PARENTID));
